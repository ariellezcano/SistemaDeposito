...
import { DetalleCompra, Modelo, OrdenCompra, Persona, TipoEquipo } from 'src/app/modelo/index.models';
import { DetalleCompraService, OrdenCompraService } from 'src/app/servicio/index.service';
  ...
  async findID() {
    try {
      if (this.id > 0) {
        ...
        if (res.status == 200) {
          this.item.compra = res.data;
          
          if(this.item.fecha_recepcion != undefined)
          this.item.fecha_recepcion = moment(this.item.fecha_recepcion).format('YYYY-MM-DD');
        }
      } else {
        this.item = new DetalleCompra();
      }
...
//control de búsqueda
  async buscar(op: any) {
    ...
    switch (op) {
      case 1:
        {
          crit = "c.compra.ordenCompraNum like '" + this.item.compra.ordenCompraNum + "' AND c.activo=true";
        }
        break;
      case 2:
        {
          crit = "c.compra.nroExpediente like '" + this.item.compra.nroExpediente + "' AND c.activo=true";
        }
        break;
      default: {
      }
